...
\section{Discussion and Conclusion}
    ...
    I'll put it here ----> Actually, the GitHub page is updated with the new file, not sure why this one is not refreshed... (???)
...
One final thing to note is that the answer to ``When does garbage stink?'' can be only answered with simulation based \textit{what if} designs (and only to a very limited degree) and thus cannot really be applied to actual real-world scenarios. Our hope is, then, to avoid garbage altogether by filtering it out before it meets our models. GIGO.
 ...
 %   The aims of the current paper were twofold; first we provide a systematic review of how the matter of validation is approached by the field at large, and second, we aim to provide simulation-based evidence of how various decisions taken based on the \enquote{quality} of manually coded materials as a \enquote{gold-standard} in the training and validation phases of a study that relies on machine learning applications can have widely different implications. To this end, we first presented the results of a meta study of relevant journal articles, as well as designed a set of Monte Carlo simulations.
    
    
%    I have the whole conclusion thing on my computer. Didn't insert it here :) -- sounds great! go ahead I will wait until tomorrow then :-)
    
%    Do i put it right here? or bellow? -- well, as you wish. I will look it over regardless and would make an edit, so in a way that makes most sense to you!
    
%    Yeah, it's not final. A draft, but I thought maybe the "skeleton" is somewhat there...
